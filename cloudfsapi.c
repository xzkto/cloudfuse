...
int cloudfs_list_directory(const char *path, dir_entry **dir_list) {
    ...
    *dir_list = NULL;
    ...
    do {
        ...
        if (marker[0]) {
            snprintf(marked_container, sizeof (marked_container), "%s&marker=%s", container, marker);
            response = send_request("GET", marked_container, NULL, xmlctx, NULL);
        } else {
                    ...
                    for (anode = onode->children; anode; anode = anode->next) {
                        ...
                        if (!strcasecmp((const char *) anode->name, "name")) {
                            ...
                            if (asprintf(&(de->full_name), "%s/%s", path, de->name) < 0)
                                de->full_name = NULL;
                            strncpy(marker, content, sizeof (marker));
                        }
    ...
    } while ((entry_count >= 10000) && marker[0]);
